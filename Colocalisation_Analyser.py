...
def build_parameters(params):
    ...
    parameters.append("%s : %s" % ("Permutations ",
                                   params[PARAM_PERMUTATIONS]))
    parameters.append("%s : %s" % ("Minimum shift", params[PARAM_MIN_SHIFT]))
    parameters.append("%s : %s" % ("Maximum shift", params[PARAM_MAX_SHIFT]))
    parameters.append("%s : %g" % ("Significance ",
                                   params[PARAM_SIGNIFICANCE]))
    return parameters
...
def create_report(conn, results, params):
    ...
    """
    report = ["Project,Dataset,Image ID,Name,p,Method,Frame,"
              "Ch1,Ch2,Ch3,n,Area,M1,Sig,M2,Sig,R,Sig"]
    for image_id, result in results.iteritems():
        img = conn.getObject('Image', image_id)
        if not img:
            continue
...
def find_channel_index(img, c):
    """
    Find the channel index (zero based) corresponding to the channel
    name/index. Returns -1 if no channel found.
...
def check_parameters(conn, images, params):
    """
    For each image check that the parameters for the channels and frames are
    OK. Return False if any check failed.
    ...
    if params[PARAM_MAX_SHIFT] <= params[PARAM_MIN_SHIFT]:
        print ("ERROR: Maximum shift (%d) is not greater than minimum shift "
               "(%d)" % (params[PARAM_MAX_SHIFT], params[PARAM_MIN_SHIFT]))
        result = False
